/**
 * Input Sanitization Service for email templates and user inputs
...
export class InputSanitizationService {
  ...
  // Safe HTML tags for email templates (very limited set)
  private static readonly SAFE_EMAIL_TAGS = [
  ...
  static sanitizeMessage(message: string): string {
    ...
    // Remove most HTML tags but allow some safe formatting
    sanitized = this.sanitizeHtml(sanitized, this.SAFE_EMAIL_TAGS);
   ...
   */
  private static sanitizeHtml(input: string, allowedTags: string[]): string {
    ...
    let sanitized = input.replace(tagRegex, '');
    
    // Remove attributes from allowed tags (for security)
    const attributeRegex = new RegExp(`<(${allowedPattern})(?:\s[^>]*)?>`, 'gi');
  ...
  /**
   * Validate and sanitize invitation request data
   */
  static sanitizeInvitationRequest(data: {
    parent_name: string;
    parent_email: string;
    child_name: string;
    child_age: number;
    ...
    return {
      parent_name: this.sanitizeName(data.parent_name),
      parent_email: this.sanitizeEmail(data.parent_email),
      child_name: this.sanitizeName(data.child_name),
      child_age: this.sanitizeNumber(data.child_age),
      message: data.message ? this.sanitizeMessage(data.message) : null
    };
  }
  ...
  static getSanitizationStats(original: string, sanitized: string): {
    ...
    const charactersRemoved = originalLength - sanitizedLength;
    ...
    return {
      ...
      sanitizedLength,
      charactersRemoved,
      percentageReduced: Math.round(percentageReduced * 100) / 100
    };
  }
}
