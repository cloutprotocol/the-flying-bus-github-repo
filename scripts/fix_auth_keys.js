
const fs = require('fs');
const path = require('path');
const { execSync } = require('child_process');

const envPath = path.join(process.cwd(), '.env.local');

const KEY_1 = `-----BEGIN PRIVATE KEY-----

ktkZUbHKUfn9ecTs62K2itQ`.trim();

const KEY_2 = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`.trim();

// Format requires: "key" not 'key' if we want to be standard, but usually just proper newlines.
// For .env, we'll replace actual newlines with literal \n
const formatKey = (k) => `"${k.replace(/\n/g, '\\n')}"`;

const key1Formatted = formatKey(KEY_1);
const key2Formatted = formatKey(KEY_2);

// Update .env.local
let envContent = '';
try {
    envContent = fs.readFileSync(envPath, 'utf8');
} catch (e) {
    console.log('.env.local not found, creating new one');
}

// Remove old entries
const lines = envContent.split('\n').filter(line =>
    !line.startsWith('CONVEX_AUTH_PRIVATE_KEY=') &&
    !line.startsWith('JWT_PRIVATE_KEY=') &&
    !line.startsWith('JWKS=')
);

// Append new entries
lines.push(`CONVEX_AUTH_PRIVATE_KEY=${key1Formatted}`);
lines.push(`JWT_PRIVATE_KEY=${key2Formatted}`);

fs.writeFileSync(envPath, lines.join('\n') + '\n');
console.log('✅ Updated .env.local with new keys');

// Also try to set remote envs via CLI using the single-line formatted strings
try {
    console.log('Setting remote CONVEX_AUTH_PRIVATE_KEY...');
    execSync(`npx convex env set CONVEX_AUTH_PRIVATE_KEY=${key1Formatted}`, { stdio: 'inherit' });

    console.log('Setting remote JWT_PRIVATE_KEY...');
    execSync(`npx convex env set JWT_PRIVATE_KEY=${key2Formatted}`, { stdio: 'inherit' });

    // Just in case JWKS is also checked by some logic
    console.log('Setting remote JWKS (same as JWT_PRIVATE_KEY)...');
    execSync(`npx convex env set JWKS=${key2Formatted}`, { stdio: 'inherit' });

} catch (e) {
    console.error('⚠️ Failed to set remote env vars:', e.message);
    // Don't exit error, as local dev might work now
}
